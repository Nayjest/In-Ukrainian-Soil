...
public class FlexibleMusicManager : MonoBehaviour
{
    ...
    public PlayListType CurPlayListType = PlayListType.Everything;
    ...
    {
        ChoosePlayList(scene.name, true);        
    }

    void ChoosePlayList(string sceneName, bool andPlay = false)
    {        
        var target = CurPlayListType;
        PlayList newOne = null;
        foreach(var p in PlayLists)
        {
            if (p.SceneNames.Contains(sceneName))
            {
        ...
        if (target == CurPlayListType) return;
        CurPlayListType = target;
        playList = newOne.Tracks;
        if (andPlay)
        {
            Rewind();
            Shuffle();
    ...
    public static FlexibleMusicManager Inst;
    ...
    void Start()
    {
        //SetVolume(PlayerPrefs.GetFloat("MusicVolume", 1));
        //Mixer.SetFloat("MusicVolume", Mathf.Log(UserPrefs.Inst.Settings.MusicVolume) * 20);
        //Mixer.SetFloat("SoundVolume", Mathf.Log(UserPrefs.Inst.Settings.SoundVolume) * 20);                
        if (loadFromResources)
        {
            LoadClipsFromResources();
        }
        ChoosePlayList(SceneManager.GetActiveScene().name);
        Rewind();
        Shuffle();
        if (playOnAwake)
        {
